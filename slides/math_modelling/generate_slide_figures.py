...
def copy_existing_outputs() -> None:
    ...
    make_intro_fundamental_diagram()


def make_intro_fundamental_diagram() -> None:
    csv_path = ROOT / "output" / "exp2.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"Missing source data: {csv_path}")

    df = pd.read_csv(csv_path)
    baseline = df[(df["remote_action"].astype(int) == 0) & (df["b"].round(2) == round(B, 2))]
    baseline = baseline.sort_values("rho")

    _set_rc("modern")
    fig, ax = _new_figure("modern")
    ax.plot(baseline["rho"], baseline["mean_velocity"], color=BLUE, linewidth=2.1, alpha=0.9, zorder=2)
    ax.scatter(
        baseline["rho"],
        baseline["mean_velocity"],
        s=44,
        marker="o",
        color=BLUE,
        edgecolor="white",
        linewidth=1.05,
        label=r"our hard-body run, $b=0.56$",
        zorder=3,
    )
    ...
    _add_modern_header(
        ...
        "Group simulation output used as the fundamental-diagram reference",
    )
    ...
    fig.savefig(SLIDE_DIR / "fundamental_diagram.png", bbox_inches="tight", pad_inches=0.08, dpi=320)
    plt.close(fig)
...
def enforce_no_overlap_fast(
    ...
    while pending and guard < n_pedestrians * n_pedestrians * 3:
        ...
        needed = body_length + speed_space_coeff * v_new[i]
...
def simulate_fast(
    *,
    ...
    x = np.arange(n_pedestrians, dtype=float) * body_length
    velocities: list[float] = []

    for step in range(T_RELAX + T_MEASURE):
        front_x = np.roll(x, -1)
        gaps = (front_x - x) % corridor_length
        needed = body_length + speed_space_coeff * v
        forces = np.where(gaps > needed, (v0 - v) / relaxation_time, -v / DT)
        next_v = np.clip(v + DT * forces, 0, v0)
        next_x = (x + next_v * DT) % corridor_length
        next_x, next_v = enforce_no_overlap_fast(
            x,
            next_x,
            next_v,
            corridor_length,
...
def simulate_case(args: tuple[int, float, float, float, int, int, float, float, float]) -> dict[str, float] | None:
    (
        density_index,
        target_density,
        corridor_length,
    ...
    if body_length * n_pedestrians >= corridor_length:
        return None
    rho, mean_velocity = simulate_fast(
        ...
        speed_space_coeff=speed_space_coeff,
...
def run_curve(
    *,
    ...
    grouped = (
        df.groupby(["rho", "N", "L", "v0_std", "a", "b", "tau"], as_index=False)["mean_velocity"]
        .mean()
        .sort_values("rho")
    )
    return [
...
def make_body_length_data(force: bool) -> Path:
    path = DATA_DIR / "body_length.csv"
    if path.exists() and not force:
        return path

    DATA_DIR.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, float]] = []
    for series_index, body_length in enumerate([0.30, 0.36, 0.42]):
        rows.extend(
            run_curve(
                densities=DENSITIES,
...
def generate(force: bool, copy_existing: bool = True) -> None:
    ...
    body_length_csv = make_body_length_data(force)
    plot_grouped_curves(
        body_length_csv,
        group_column="a",
        output_path=SLIDE_DIR / "body_length.png",
        title="Parameter sensitivity",
        subtitle=r"Varying baseline required space $a$ in $d=a+bv$",
        label_prefix=r"$a$",
        label_format=".2f",
    )
